...
play_orig <- read.csv("D:/website/eeeeevahe.github.io/files/google/playStore5.csv")
play <- play_orig
str(play)
```
...
The regression model needs to be further improved for better fit because it has the ability to predict but we can’t say that this is the best predictive model. The “Normal Q-Q” plot is saying that our errors are not normally-distributed so I draw a histogram to see the distribution directly. The residual vs leverage plot saying that our data have outliers problems.
...
change the size of L and XL into 1 which means this app will be a hit and others to 0 which means this app will less likely to be a hit.
...
library(DMwR)
playmodel <- play %>% 
  mutate(Installs = ifelse((Installs=="L")| (Installs=="XL"),1,0),
         Installs = as.factor(Installs),
         Rating = as.factor(Rating)) %>% 
  dplyr::select(-Current.Ver,-Android.Ver,-Last.Updated,-App)

set.seed(1234)
sample_set <- sample(nrow(playmodel), round(nrow(playmodel)*.7), replace = FALSE)
play_train <- playmodel[sample_set, ]
play_test <- playmodel[-sample_set, ]
# Generate new balanced training data.
set.seed(1234)
play_train <- SMOTE(Installs ~ ., play_train, perc.over = 200, perc.under =100)
...
# evaluate
tree_pred <- predict(tree_mod, play_test,  type = "class")

# Using our predictions, we can construct a Confusion Matrix.
tree_pred_table <- table(play_test$Installs, tree_pred)
tree_pred_table

# What is our accuracy?
tree_pred_accuracy <- sum(diag(tree_pred_table)) / nrow(play_test)
tree_pred_accuracy
```

The accuracy of the tree is 94%.
